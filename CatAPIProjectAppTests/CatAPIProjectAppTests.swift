//
//  CatAPIProjectAppTests.swift
//  CatAPIProjectAppTests
//
...
import XCTest
import Combine
@testable import CatAPIProjectApp

class CatAPIProjectAppTests: XCTestCase {
    ...
    func test_loading_error() {
        let result = Result<[Breed], APIError>.failure(.badURL)
        let fetcher = BreedFetcher(service: APIMockService(result: result))
